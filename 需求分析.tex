...
\hline
\textbf{活动图} & \includegraphics[width=0.7\textwidth, height=0.47\textheight, keepaspectratio]{BorrowBookActivity.png} \\
\hline
\end{tabular}
\setcounter{table}{0} 
\caption{借阅图书用例分析}
...
\hline
\textbf{活动图} & \includegraphics[width=0.7\textwidth, height=0.52\textheight, keepaspectratio]{ReturnBookActivity.png} \\
\hline
\end{tabular}
\caption{归还图书用例分析}
\end{table}
...
\hline
\textbf{活动图} & \includegraphics[width=0.7\textwidth, height=0.6\textheight, keepaspectratio]{RecordsViewActivity.png} \\
\hline
\end{tabular}
\caption{查看借阅记录用例分析}
\end{table}
...
\hline
\textbf{活动图} & \includegraphics[width=0.7\textwidth, height=0.58\textheight, keepaspectratio]{QueryBookActivity.png} \\
\hline
\end{tabular}
\caption{查询图书用例分析}
\end{table}
...
4. 若数据存储失败，返回提示，回滚操作 \\
\hline
\textbf{活动图} & \includegraphics[width=0.7\textwidth, height=0.57\textheight, keepaspectratio]{AddBookActivity.png} \\
\hline
\end{tabular}
\caption{新增图书用例分析}
\end{table}
...
\hline
\textbf{活动图} & \includegraphics[width=0.7\textwidth, height=0.56\textheight, keepaspectratio]{ChangeBookActivity.png} \\
\hline
\end{tabular}
\caption{修改图书信息用例分析}
\end{table}
...
\hline
\textbf{活动图} & \includegraphics[width=0.7\textwidth, height=1.2\textheight, keepaspectratio]{DeleteBookActivity.png} \\
\hline
\end{tabular}
\caption{删除图书用例分析}
\end{table}
...
数据流图（如图3.2.1所示）完整呈现了系统的数据边界与核心数据流转链路，识别了以下核心元素：

\begin{enumerate}
\item \textbf{外部实体}：数据的发起方与接收方，包括普通用户、图书管理员、系统管理员三类角色，是系统数据的源头与终点
\item \textbf{核心数据加工过程}：系统对数据的处理逻辑单元，包括图书查阅/借阅/归还加工、图书信息维护加工、系统数据维护加工、数据查找与匹配加工四大单元，对应系统的核心业务逻辑处理环节
\item \textbf{核心数据存储}：系统的持久化数据载体，包括本地书籍数据存储、本地借阅记录数据存储，分别对应图书基础信息库与借阅流水信息库
\end{enumerate}

\textbf{核心数据流向}：
\begin{itemize}
\item 普通用户发起图书查阅、借阅、归还请求，系统从本地书籍数据存储中获取图书数据，从本地借阅记录存储中获取/更新借阅数据，最终向用户返回操作结果与数据信息
\item 图书管理员发起图书信息新增、修改、删除请求，系统对本地书籍数据存储中的数据进行维护更新，向管理员返回操作结果
\item 系统管理员发起系统维护请求，系统对本地存储的全量数据进行管理与维护，保障数据的完整性与可用性
\end{itemize}
\begin{figure}[H]
    \centering
...
\item \textbf{核心处理单元}：
  ...
  \item Process 单元：程序的核心逻辑处理单元，包括库存检查、图书数据增删改查处理、借阅记录生成与更新处理等，划分为 BookManage（图书管理）、BorrowManage（借阅管理）两大子模块
