...
\subsection{2.2 核心模块划分}

系统划分为4个核心功能模块，完整覆盖需求文档所有业务场景，模块职责如下：

1. 认证授权模块：负责用户登录、身份校验、会话管理、权限控制，是系统的安全入口

2. 图书管理模块：负责图书信息的全生命周期管理，包括图书新增、查询、修改、删除、库存管理

3. 借阅管理模块：负责图书借阅、归还全流程处理，借阅记录的多维度查询与状态管理

4. 系统管理模块：负责系统数据备份、数据恢复、系统配置管理，保障系统数据安全与稳定运行

\subsection{2.3 角色权限矩阵}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|l|l|l|l|}
        ...
        功能点编号 & 功能点名称 & 核心目标 & 前置条件 & 后置条件 \\
        \hline
        Borrow-001 & 图书借阅 & 完成图书借阅操作，扣减库存，生成借阅记录 & 用户已登录，目标图书存在且库存>0 & 生成有效借阅记录，状态标记为未归还，对应图书库存扣减1，数据同步更新 \\
        \hline
        Borrow-002 & 图书归还 & 完成图书归还操作，恢复库存，更新借阅记录状态 & 用户已登录，存在对应未归还的有效借阅记录 & 借阅记录更新归还时间与已归还状态，对应图书库存增加1，数据同步更新 \\
        \hline
...
\section{本核心用例图围绕图书管理系统的普通用户与图书管理员两类角色，完整呈现了图书借阅、归还、信息管理、记录查询等核心业务功能}
...
• 统一响应格式：{"code": 状态码, "msg": 提示信息, "data": 响应数据}

• 所有业务接口请求头必须携带Authorization: Bearer {Token}，由拦截器统一校验

• 数据格式：JSON，字符编码：UTF-8

\subsection{5.1 认证授权模块接口}

\subsubsection{5.1.1 验证码接口}
...
1. 前提：集成Kaptcha验证码工具，引入对应依赖包，完成配置类编写，Redis缓存服务正常运行

2. 调用Kaptcha的createText()方法生成4位数字+字母组合的验证码文本codeText

3. 以入参imageCodeToken为key，codeText为value，存入Redis，设置过期时间300秒

4. 调用Kaptcha的createImage(codeText)方法生成验证码图片
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|}
        \hline
        Python\\
        def get\_image\_code(imageCodeToken: str):\\
        \ \ \ \ code\_text = kaptcha.create\_text()\\
        \ \ \ \  存入Redis，5分钟过期\\
        \ \ \ \ redis\_client.setex(imageCodeToken, 300, code\_text)\\
        \ \ \ \  生成图片\\
        \ \ \ \ image = kaptcha.create\_image(code\_text)\\
        \ \ \ \  返回图片流\\
        \ \ \ \ return Response(image, content\_type="image/jpeg") \\
        \hline
    \end{tabular}
\end{table}
\subsubsection{5.1.2 登录接口}
...
1. 对前端传入的加密密码进行二次加盐加密，生成最终校验密码

2. 验证码校验：根据imageCodeToken从Redis中获取验证码文本

￮ 获取不到：返回错误码10001，提示「验证码已过期」，终止流程

￮ 获取到后与传入的imageCode比对，不匹配：返回错误码10002，提示「验证码错误」，终止流程

3. 验证码校验通过后，删除Redis中对应的imageCodeToken，防止验证码重复使用

4. 用户身份校验：根据loginName从数据库查询用户信息

￮ 查询无结果：记录安全日志，返回错误码10003，提示「用户名不存在或密码错误」，终止流程

￮ 查询到用户信息，比对二次加密后的密码与数据库存储密码

▪ 不匹配：记录安全日志，返回错误码10003，提示「用户名不存在或密码错误」，终止流程

▪ 匹配：登录身份校验通过

5. 根据用户角色，加载对应的权限资源列表

6. 组装用户登录信息LoginUserDto，包含用户ID、用户名、角色、权限、登录时间

7. 生成32位唯一UUID作为登录Token，设置有效期2小时

8. 以Token为key，LoginUserDto为value，存入Redis，设置过期时间7200秒
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|p{14cm}|}
        \hline
        Python\\
        def user\_login(login\_dto: LoginDto):\\
            密码二次加密\\
            encrypt\_pwd = password\_encrypt(login\_dto.password, SALT)\\
            验证码校验\\
            cache\_code = redis\_client.get(login\_dto.imageCodeToken)\\
            if not cache\_code:\\
                \ \ \ \ return Response(code=10001, msg="验证码已过期")\\
            if cache\_code.lower() != login\_dto.imageCode.lower():\\
                return Response(code=10002, msg="验证码错误")\\
            删除验证码\\
            redis\_client.delete(login\_dto.imageCodeToken)\\
            用户身份校验\\
            user = user\_dao.get\_by\_login\_name(login\_dto.loginName)\\
            if not user or user.password != encrypt\_pwd:\\
                \ \ \ \ logger.warning(f"登录失败，用户名\{login\_dto.loginName\}，密码错误")\\
                return Response(code=10003, msg="用户名不存在或密码错误")\\
            加载权限\\
            permissions = permission\_service.get\_by\_role(user.role)\\
            生成Token\\
            token = uuid.uuid4().hex\\
            组装用户信息\\
            login\_user = LoginUserDto(\\
                userId=user.id,\\
            ...
            存入Redis\\
            redis\_client.setex(token, 7200, json.dumps(login\_user.dict()))\\
            返回结果\\
...
1. 从请求头中获取用户Token，解析并校验Token有效性

2. 从Redis中删除该Token对应的用户登录信息

3. 返回登出成功结果

\subsection{5.2 图书管理模块接口}

\subsubsection{5.2.1 图书多条件查询接口}
...
1. 拦截器校验用户Token与图书查询权限，无权限直接拦截

2. 校验入参合法性，queryType不在指定范围返回参数错误，isbn查询时keyword为空返回参数错误

3. 根据查询条件构建查询语句，执行分页查询

4. 组装查询结果，返回分页数据

\subsubsection{5.2.2 图书新增接口}
...
1. 拦截器校验用户Token与图书管理权限，无权限直接拦截

2. 入参合法性校验：必填项非空校验，stock与totalCount≥0校验

3. ISBN唯一性校验：查询数据库是否存在该ISBN的图书，已存在返回错误码20001，提示「ISBN已存在」

4. 校验通过后，组装图书实体对象，写入数据库

5. 写入成功返回新增成功，写入失败返回操作失败

\subsubsection{5.2.3 图书修改接口}
...
3. 图书存在性校验：根据ID查询图书，不存在返回错误码20002，提示「图书不存在」

4. 校验通过后，更新数据库中对应图书的信息

5. 更新成功返回修改成功，更新失败返回操作失败

\subsubsection{5.2.4 图书删除接口}
...
2. 图书存在性校验：根据ID查询图书，不存在返回错误码20002，提示「图书不存在」

3. 借阅记录校验：查询该图书ISBN是否存在未归还的借阅记录，存在返回错误码20003，提示「该图书存在未归还借阅记录，无法删除」

4. 校验通过后，执行数据库删除操作，删除该图书数据

5. 删除成功返回删除成功，删除失败返回操作失败

\subsection{5.3 借阅管理模块接口}

\subsubsection{5.3.1 图书借阅接口}
...
1. 拦截器校验用户Token与借阅操作权限，无权限直接拦截

2. 入参合法性校验：必填项非空校验

3. 图书存在性校验：根据ISBN查询图书，不存在返回错误码30001，提示「图书不存在」

4. 库存校验：图书库存≤0，返回错误码30002，提示「图书无可用库存，无法借阅」
...
6. 事务执行成功，返回借阅成功；事务执行失败，回滚所有操作，返回借阅失败

\subsubsection{5.3.2 图书归还接口}
...
3. 借阅记录校验：根据ID查询借阅记录，不存在返回错误码30003，提示「借阅记录不存在」；记录状态为「已归还」，返回错误码30004，提示「该图书已归还」

4. 图书存在性校验：根据ISBN查询图书，不存在返回错误码30001，提示「图书不存在」
...
6. 事务执行成功，返回归还成功；事务执行失败，回滚所有操作，返回归还失败

\subsubsection{5.3.3 借阅记录查询接口}
...
1. 拦截器校验用户Token与对应查询权限，无权限直接拦截

2. 入参合法性校验：queryType不在指定范围返回参数错误，borrower查询时borrowerName为空返回参数错误

3. 根据用户角色与查询类型，构建查询条件：

￮ 普通用户personal查询：仅查询当前用户的借阅记录

￮ unreturned查询：仅筛选状态为未归还的借阅记录

4. 执行分页查询，关联图书表匹配书名信息

5. 组装查询结果，返回分页数据

\subsection{5.4 系统管理模块接口}

\subsubsection{5.4.1 数据备份接口}
...
1. 拦截器校验用户Token与系统维护权限，无权限直接拦截

2. 读取数据库全量数据：用户表、图书表、借阅记录表、系统配置表

3. 生成JSON格式备份数据，使用AES加密，生成带时间戳的备份文件

4. 记录备份日志，包含备份时间、操作人、备份版本、文件大小

5. 返回备份成功结果与备份文件下载地址

\subsubsection{5.4.2 数据恢复接口}
...
2. 校验备份文件合法性：文件格式校验、解密校验、数据完整性校验

3. 校验不通过，返回错误码40001，提示「备份文件非法，无法恢复」

4. 校验通过后，先备份当前数据库数据，再执行数据恢复操作，全量覆盖数据库表数据

5. 恢复成功，记录恢复日志，返回恢复成功；恢复失败，回滚到恢复前的数据，返回恢复失败
...
\subsection{6.1 实体类（Entities）}

实体类为系统核心数据载体，位于实体层，无业务逻辑，仅封装属性与基础行为，被所有上层依赖。

\subsubsection{6.1.1 图书类（Book）}
...
\subsubsection{6.1.2 借阅记录类（BorrowRecord）}
...
\subsection{6.2 数据访问类（DAO）}

数据访问类位于数据访问层，封装本地存储/数据库的读写操作，屏蔽存储细节，为业务层提供统一的数据访问接口。

\subsubsection{6.2.1 基础DAO类（BaseDAO）}
...
\subsubsection{6.3.2 借阅规则类（BorrowRule）}
...
\subsection{6.4 业务管理类（Managers）}

业务管理类位于业务逻辑层，是核心业务规则的执行单元，封装对应模块的全量业务逻辑，编排业务流程，处理业务异常。

\subsubsection{6.4.1 图书管理器（BookManager）}

依赖BookDAO、StockRule、BorrowRule，封装图书管理全量业务逻辑，包括图书新增、查询、修改、删除的核心业务流程处理。

\subsubsection{6.4.2 借阅管理器（BorrowManager）}

依赖BorrowRecordDAO、BookDAO、BorrowRule、StockRule，封装借阅管理全量业务逻辑，包括图书借阅、归还、借阅记录查询的核心业务流程处理，事务控制。

\subsubsection{6.4.3 用户管理器（UserManager）}

依赖UserDAO，封装用户身份校验、权限加载、用户信息管理的核心业务逻辑。

\subsubsection{6.4.4 系统管理器（SystemManager）}

依赖所有DAO类，封装系统数据备份、数据恢复、系统配置管理的核心业务逻辑。

\subsection{6.5 工具类（Utils）}

工具类封装系统通用工具方法，实现代码复用，所有方法为静态方法，无状态，可全局调用。

1. 加密工具类（EncryptUtil）：封装密码加密、AES加解密、MD5摘要等方法

2. 验证码工具类（CaptchaUtil）：封装验证码生成、图片生成方法

3. JWT工具类（JwtUtil）：封装Token生成、校验、解析方法

4. 日期工具类（DateUtil）：封装日期格式化、时间差计算等方法

5. 分页工具类（PageUtil）：封装分页参数处理、分页结果组装方法

\section{7 页面详细设计}

\subsection{7.1 通用页面规范}

1. 页面布局：顶部导航栏+左侧菜单栏+主内容区的三段式布局，适配1920*1080及以上主流分辨率

2. 导航栏固定展示：系统名称、当前登录用户、角色、登出按钮

3. 菜单栏根据用户角色动态展示有权限的菜单，无权限的菜单隐藏

4. 所有表单提交前必须执行前端非空校验与格式校验，提交按钮点击后置灰，防止重复提交

5. 所有操作结果必须给出明确的成功/失败提示，异常场景给出可理解的错误信息

\subsection{7.2 登录页面}

\subsubsection{7.2.1 页面入口}

系统根路径/，未登录用户访问任何页面均重定向至登录页

\subsubsection{7.2.2 页面元素}

1. 页面主体：系统logo、系统名称「图书管理系统」
...
4. 任一校验不通过，登录按钮置灰，对应输入框下方给出红色提示文字

\subsubsection{7.2.4 处理逻辑与后端交互}

1. 页面初始化完成后，前端生成32位随机imageCodeToken，调用验证码接口，渲染验证码图片

2. 验证码图片点击事件：重新生成imageCodeToken，调用验证码接口，刷新图片
...
\subsubsection{7.3.1 页面入口}

登录成功后默认跳转至/home，导航栏「首页」菜单

\subsubsection{7.3.2 页面元素与逻辑}

1. 系统欢迎语：展示当前登录用户姓名、角色

2. 系统功能介绍卡片：根据用户角色展示对应可操作的功能模块介绍与快捷入口
...
4. 快捷操作按钮：根据角色展示高频操作入口，如图书查询、借阅操作、新增图书等

\subsection{7.4 图书管理页面}

\subsubsection{7.4.1 页面入口}

左侧菜单栏「图书管理」，仅图书管理员可访问，普通用户仅可访问图书查询子页面

\subsubsection{7.4.2 页面元素}
...
4. 分页组件：展示总条数、页码切换、每页条数调整

5. 新增/编辑弹窗：包含图书所有信息的表单，含表单校验

6. 删除确认弹窗：二次确认删除操作，提示风险

\subsubsection{7.4.3 处理逻辑与后端交互}

1. 页面初始化：调用图书查询接口，加载第一页图书列表

2. 查询按钮点击事件：组装查询参数，调用图书查询接口，刷新列表

3. 新增按钮点击事件：打开新增弹窗，清空表单，用户填写后提交，调用图书新增接口，成功后关闭弹窗，刷新列表

4. 修改按钮点击事件：打开编辑弹窗，回显当前图书信息，用户修改后提交，调用图书修改接口，成功后关闭弹窗，刷新列表

5. 删除按钮点击事件：弹出二次确认弹窗，确认后调用图书删除接口，成功后刷新列表，失败弹出错误提示

\subsection{7.5 借阅管理页面}

\subsubsection{7.5.1 页面入口}

左侧菜单栏「借阅管理」，所有登录用户可访问，根据角色展示不同功能

\subsubsection{7.5.2 页面元素}
...
5. 借阅操作弹窗：ISBN、借阅人信息表单

6. 归还操作弹窗：借阅记录ID、ISBN确认表单

\subsubsection{7.5.3 处理逻辑与后端交互}

1. 页面初始化：根据用户角色调用借阅记录查询接口，加载对应列表

2. 借阅按钮点击事件：打开借阅弹窗，用户填写信息提交，调用图书借阅接口，成功后关闭弹窗，刷新列表与图书库存

3. 归还按钮点击事件：打开归还弹窗，回显借阅记录信息，用户确认后提交，调用图书归还接口，成功后关闭弹窗，刷新列表与图书库存

4. 查询按钮点击事件：组装查询参数，调用借阅记录查询接口，刷新列表

\subsection{7.6 系统管理页面}

\subsubsection{7.6.1 页面入口}

左侧菜单栏「系统管理」，仅系统维护人员可访问

\subsubsection{7.6.2 页面元素}

1. 数据备份区域：「手动备份」按钮、备份记录列表、备份文件下载按钮

2. 数据恢复区域：备份文件上传组件、「开始恢复」按钮、恢复风险提示

3. 系统运行信息区域：展示系统版本、数据库大小、运行时长、备份记录统计

\subsubsection{7.6.3 处理逻辑与后端交互}

1. 页面初始化：调用接口加载备份记录列表与系统运行信息

2. 手动备份按钮点击事件：调用数据备份接口，成功后刷新备份列表，提供文件下载

3. 恢复按钮点击事件：弹出二次确认弹窗，确认后上传备份文件，调用数据恢复接口，成功后提示用户重新登录

\section{8 数据库设计}

\subsection{8.1 数据库选型}

基于需求文档「本地存储」要求，选型SQLite 3作为系统数据库，优势如下：

1. 轻量级嵌入式数据库，无需独立安装部署，单文件存储，适配全平台操作系统

2. 完全开源免费，无版权限制，符合项目经济可行性要求

3. 支持完整的关系型数据库特性，满足系统事务、索引、关联查询需求

4. 适配Python标准库，无需额外依赖，符合技术可行性要求

\subsection{8.2 核心表结构设计}

\subsubsection{8.2.1 用户表（sys_user）}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|l|l|l|l|l|}
        ...
        \hline
        login_name & TEXT & 否 & 是 & 是 & 登录用户名 \\
        \hline
        password & TEXT & 否 & 是 & 否 & 加盐加密后的密码 \\
        \hline
        real_name & TEXT & 否 & 是 & 否 & 用户真实姓名 \\
        \hline
        role & TEXT & 否 & 是 & 否 & 用户角色：普通用户/图书管理员/系统维护人员 \\
        \hline
        create_time & DATETIME & 否 & 是 & 否 & 创建时间 \\
        \hline
        update_time & DATETIME & 否 & 是 & 否 & 更新时间 \\
        \hline
    \end{tabular}
\end{table}
...
\subsubsection{8.2.2 图书表（book_info）}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|l|l|l|l|l|}
        ...
        \hline
        total_count & INTEGER & 否 & 是 & 否 & 总馆藏数量，默认0，≥0 \\
        \hline
        create_time & DATETIME & 否 & 是 & 否 & 入库时间 \\
        \hline
        update_time & DATETIME & 否 & 是 & 否 & 更新时间 \\
        \hline
    \end{tabular}
...
索引：isbn 唯一索引；book_name 普通索引；author 普通索引

\subsubsection{8.2.3 借阅记录表（borrow_record）}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|l|l|l|l|l|}
        ...
        book_name & TEXT & 否 & 是 & 否 & 书名 \\
        \hline
        borrower_name & TEXT & 否 & 是 & 否 & 借阅人姓名 \\
        \hline
        borrower_phone & TEXT & 否 & 否 & 否 & 借阅人联系电话 \\
        \hline
        borrow_time & DATETIME & 否 & 是 & 否 & 借阅时间 \\
        \hline
        return_time & DATETIME & 否 & 否 & 否 & 归还时间，未归还为NULL \\
        \hline
...
索引：isbn 普通索引；borrower_name 普通索引；status 普通索引

\subsubsection{8.2.4 系统配置表（sys_config）}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|l|l|l|l|l|}
        ...
        id & TEXT & 是 & 是 & 是 & 配置ID \\
        \hline
        config_key & TEXT & 否 & 是 & 是 & 配置键 \\
        \hline
        config_value & TEXT & 否 & 是 & 否 & 配置值 \\
        \hline
...
\subsubsection{8.2.5 操作日志表（sys_operation_log）}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|l|l|l|l|l|}
        ...
        id & TEXT & 是 & 是 & 是 & 日志ID \\
        \hline
        user_id & TEXT & 否 & 是 & 否 & 操作人ID \\
        \hline
        user_name & TEXT & 否 & 是 & 否 & 操作人姓名 \\
        \hline
        operation_type & TEXT & 否 & 是 & 否 & 操作类型：登录/新增/修改/删除/借阅/归还/备份/恢复 \\
        \hline
        operation_content & TEXT & 否 & 是 & 否 & 操作详情 \\
        \hline
        operation_result & TEXT & 否 & 是 & 否 & 操作结果：成功/失败 \\
        \hline
        operation_time & DATETIME & 否 & 是 & 否 & 操作时间 \\
        \hline
        ip_address & TEXT & 否 & 否 & 否 & 操作IP地址 \\
        \hline
    \end{tabular}
\end{table}

\subsection{8.3 数据库初始化脚本}
...
\subsection{10.1 代码级复用}

1. 通用工具类复用：将加密、验证码、日期、分页、数据库连接等通用逻辑封装为静态工具类，全局无状态调用，所有业务模块均可复用，避免重复实现。

2. 基础类继承复用：封装BaseDAO基础数据访问类，实现通用的CRUD方法，所有业务DAO类均继承此类，复用通用数据库操作逻辑，子类仅需实现专属业务方法。

3. 通用异常处理复用：封装全局统一异常处理器，定义系统自定义异常类，所有业务模块抛出的异常均由全局处理器统一处理，复用异常日志记录、响应格式封装逻辑。

4. 通用日志处理复用：封装统一的日志工具类，定义操作日志、安全日志、错误日志的规范，所有业务模块复用日志记录方法，保证日志格式统一。

\subsection{10.2 组件级复用}

1. 前端通用组件复用：封装通用表单组件、表格组件、分页组件、弹窗组件、查询组件，所有业务页面均可复用，统一页面样式与交互逻辑，减少重复开发。

2. 权限控制组件复用：封装前端权限指令、后端登录拦截器、权限校验切面，所有接口与页面均复用权限控制逻辑，无需在每个业务中重复编写权限校验代码。

3. 参数校验组件复用：基于Pydantic封装通用参数校验基类，所有接口DTO均继承基类，复用通用非空校验、格式校验、类型校验逻辑。

4. 事务控制组件复用：封装数据库事务切面，所有需要事务控制的业务方法复用事务开启、提交、回滚逻辑，无需重复编写事务管理代码。

\subsection{10.3 规则级复用}

1. 业务规则封装复用：将库存校验、借阅校验、ISBN唯一性校验等核心业务规则，封装为独立的规则校验类，所有涉及该规则的业务场景均可复用，保证业务规则的一致性。

2. 校验规则统一管理：将前端表单校验规则与后端接口校验规则统一管理，复用同一套校验逻辑，避免前后端校验规则不一致的问题。

3. 状态流转规则复用：封装图书状态、借阅记录状态的流转规则，所有涉及状态变更的业务均复用该规则，保证状态流转的规范性。

\subsection{10.4 接口级复用}

1. 统一接口规范复用：所有接口遵循统一的请求路径、请求方式、响应格式、错误码规范，前端可复用统一的接口请求方法、响应处理逻辑、异常处理逻辑。

2. 通用接口复用：封装通用的分页查询接口、数据导出接口、字典查询接口，所有业务模块均可复用，无需重复开发同类接口。

3. 接口版本控制：采用/api/v1的路径版本控制方式，后续接口迭代可兼容旧版本接口，实现接口的向后兼容复用。

\section{11 跑批设计}

\subsection{11.1 数据自动备份跑批}

\subsubsection{11.1.1 基础信息}

• 跑批名称：系统数据自动备份跑批

• 执行周期：每日凌晨2:00执行一次

• 触发方式：定时任务自动触发，支持手动触发

• 执行环境：系统服务端，Python APScheduler定时任务框架

\subsubsection{11.1.2 处理逻辑}

1. 定时任务触发后，先校验系统当前是否有活跃的业务操作，无操作则执行备份，有操作则延迟10分钟重试，最多重试3次

2. 读取数据库全量业务数据，生成JSON格式备份数据

3. 使用AES算法加密备份数据，生成以日期为命名的备份文件，保存至指定备份目录

4. 记录备份日志，包含备份时间、备份版本、文件大小、备份状态

5. 校验备份目录中的历史备份文件，仅保留最近30天的备份文件，超过30天的自动清理

6. 备份完成后，发送系统通知给系统维护人员，备份失败则发送告警通知

\subsubsection{11.1.3 异常处理与重跑机制}

1. 备份过程中出现异常，立即记录异常日志，终止当前备份操作，不影响系统正常业务运行

2. 当日备份失败，系统将在凌晨3:00、4:00自动重试2次，3次均失败则停止重试，发送告警通知

3. 支持手动触发重跑，维护人员可在系统管理页面手动触发备份操作，补全当日备份

\subsection{11.2 历史归档数据清理跑批}

\subsubsection{11.2.1 基础信息}
...
\subsubsection{11.2.2 处理逻辑}

1. 定时任务触发后，查询1年以前的已完成归还的借阅记录，标记为归档数据

2. 将归档数据导出为备份文件，保存至归档目录，永久留存

3. 校验归档文件完整性，校验通过后，删除数据库中对应的归档借阅记录

4. 清理1年以前的操作日志数据，导出归档后删除数据库中对应数据

5. 记录清理日志，包含清理时间、清理数据量、归档文件信息

6. 清理完成后，执行数据库VACUUM操作，释放数据库存储空间，优化数据库性能

\subsubsection{11.2.3 异常处理与重跑机制}

1. 清理过程中出现异常，立即回滚数据删除操作，记录异常日志，终止跑批

2. 支持手动触发重跑，维护人员可手动执行归档清理操作

3. 数据清理前必须完成全量数据备份，备份失败则终止清理操作

\section{12 安全设计}

\subsection{12.1 认证与授权安全}

1. 身份认证机制：基于Token的无状态认证机制，参考单点登录设计文档，用户登录成功后生成唯一Token，存入Redis设置2小时过期时间，超时自动登出。

2. 权限控制机制：基于RBAC角色权限模型，严格按照角色权限矩阵控制用户可访问的页面与接口，无权限请求直接拦截，前端隐藏无权限菜单与按钮。

3. 登录安全防护：所有登录操作均记录操作日志，包含登录IP、时间、结果

\subsection{12.2 数据加密安全}

1. 传输加密：前端密码采用RSA非对称加密后传输，后端二次加盐加密，防止密码在传输过程中泄露。

2. 存储加密：备份文件采用AES对称加密存储，防止备份文件数据泄露；用户敏感信息加密存储，不存储明文敏感数据。

3. 加密算法规范：采用行业标准加密算法，密码加密使用MD5+盐值，非对称加密使用RSA2048，对称加密使用AES-128，不使用自定义不安全加密算法。

\subsection{12.3 输入校验与防注入}

1. 全链路参数校验：前端表单校验+后端接口参数校验+数据库参数化查询，三层校验机制，防止非法参数传入。

2. SQL注入防护：所有数据库查询均采用参数化查询，不拼接SQL语句，防止SQL注入攻击。

3. XSS防护：所有前端输入的内容均进行转义处理，后端存储前进行特殊字符过滤，防止XSS跨站脚本攻击。

4. CSRF防护：所有POST/PUT/DELETE请求均需携带Token，前端请求头统一携带，后端拦截器校验，防止CSRF跨站请求伪造攻击。

\subsection{12.4 敏感数据处理}

1. 敏感数据脱敏：用户手机号、身份证号等敏感信息，前端展示时进行脱敏处理，仅展示部分字符。

2. 敏感操作审计：所有数据删除、数据恢复、用户管理等敏感操作，均记录详细的操作日志，包含操作人、操作时间、操作内容、操作IP、操作结果，可追溯可审计。

3. 数据访问控制：普通用户仅可访问个人相关数据，不可访问其他用户的借阅记录，图书管理员不可访问系统管理相关数据，严格控制数据访问边界。

\subsection{12.5 业务安全规则}

1. 图书删除校验：存在未归还借阅记录的图书，禁止删除，防止数据丢失与业务异常。

2. 库存安全校验：库存扣减与恢复均在数据库事务中执行，防止超借、库存负数等异常情况。

3. 借阅记录不可篡改：借阅记录生成后，仅可更新归还时间与状态，不可修改借阅人、ISBN、借阅时间等核心信息，保证借阅记录的真实性与完整性。

\section{13 性能设计}

\subsection{13.1 查询性能优化}

1. 索引优化：为所有高频查询字段建立合适的索引，包括ISBN、书名、作者、借阅人、借阅状态、登录名等，避免全表扫描，提升查询效率。

2. 分页查询优化：所有列表查询均采用分页查询，限制每页最大条数，避免全量数据查询导致的内存占用过高与响应缓慢问题。

3. 关联查询优化：减少多表关联查询，借阅记录列表提前冗余书名信息，避免查询时关联图书表。

4. 缓存优化：高频访问的系统配置、图书分类数据，存入Redis缓存，减少数据库查询次数。

\subsection{13.2 接口响应优化}

1. 接口超时控制：所有接口设置最大响应时间，慢查询接口记录告警日志，优化处理逻辑。

2. 非核心逻辑异步处理：操作日志记录、备份文件生成、通知发送等非核心逻辑，采用异步线程处理，不阻塞主业务流程，提升接口响应速度。

3. 数据压缩传输：接口响应数据采用Gzip压缩，减少网络传输数据量，提升前端加载速度。

4. 拦截器优化：登录校验、权限校验等拦截器逻辑采用Redis缓存查询，避免每次请求都查询数据库，减少接口处理耗时。

\subsection{13.3 大数据量处理优化}

1. 数据归档清理：通过定时跑批，定期清理历史归档数据，控制单表数据量，避免单表数据量过大导致的查询性能下降。

2. 批量操作优化：批量新增图书等批量操作，采用批量写入数据库的方式，减少数据库IO次数，提升处理效率。

3. 流式处理：数据备份、数据导出等大数据量操作，采用流式读写方式，避免全量数据加载到内存导致的内存溢出问题。

\subsection{13.4 存储性能优化}

1. 数据库定期优化：每月定时跑批执行数据库VACUUM操作，整理数据库文件碎片，释放存储空间，提升数据库读写性能。

2. 数据库连接优化：采用数据库连接池管理数据库连接，避免频繁创建与关闭数据库连接带来的性能开销。

3. 读写分离设计：预留读写分离扩展能力，查询操作走从库，写入操作走主库，为后续系统扩容提供支持。
...
\subsection{17.1 部署方式}

系统采用前后端分离部署方式，支持Windows、macOS、Linux全平台部署，提供两种部署方案：

\subsubsection{17.1.1 一键部署方案（推荐）}
